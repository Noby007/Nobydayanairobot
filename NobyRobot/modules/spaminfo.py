...
def lookup(update, context):
    ...
    if response is True:
        ...
        stats += f'     *Language*: `{a.get("results").get("language_prediction").get("language")}`\n'
        ...
        if a.get("results").get("attributes").get("is_blacklisted") is True:
            stats += f'     *Reason*: `{a.get("results").get("attributes").get("blacklist_reason")}`\n'
            stats += f'     *Flag*: `{a.get("results").get("attributes").get("blacklist_flag")}`\n'
        stats += f'*PTID*:\n`{a.get("results").get("private_telegram_id")}`\n'
        message.reply_text(
            stats, parse_mode=telegram.ParseMode.MARKDOWN, disable_web_page_preview=True
        )
    else:
        message.reply_text(
